 <main class="flex-1 overflow-auto">
     <div class="p-8 space-y-8">
         <div class="grid grid-cols-1 gap-8">
             <form enctype="multipart/form-data" method="post" action="<?= route_to('admin/update_hero') ?>" data-slot="card" class="text-card-foreground flex flex-col gap-6 rounded-xl border shadow-sm p-8 bg-card border-border">
                 ...
                 <div class="space-y-6">
                     ...
                     <div>
                         ...
                         <input id="fullName" value="<?= $hero['name'] ?>" name="fullName" type="text" class="w-full px-4 py-3 bg-input border border-border rounded-lg text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" placeholder="Your name" value="John Doe" />
                     </div>
                     <div>
                         <label class="block text-sm font-medium text-foreground mb-2">Job Title</label>
                         <input name="jobTitle" value="<?= $hero['job_title'] ?>" id="jobTitle" type="text" class="w-full px-4 py-3 bg-input border border-border rounded-lg text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" placeholder="Your job title" value="Full-Stack Developer" />
                     </div>
                     <div>
                         <label class="block text-sm font-medium text-foreground mb-2">Experience Year</label>
                         <input name="experienceYear" value="<?= $hero['experience'] ?>" id="jobTitle" type="number" class="w-full px-4 py-3 bg-input border border-border rounded-lg text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" placeholder="Your job title" value="Full-Stack Developer" />
                     </div>
                     <div>
                         <label class="block text-sm font-medium text-foreground mb-2">Client Satisfation Rate (%)</label>
                         <input name="satisfaction" value="<?= $hero['satisfaction'] ?>" id="jobTitle" type="number" class="w-full px-4 py-3 bg-input border border-border rounded-lg text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent" placeholder="Your job title" value="Full-Stack Developer" />
                     </div>
                     <div>
                         <label class="block text-sm font-medium text-foreground mb-2">Short Bio</label>
                         <textarea name="shortBio" id="shortBio" class="w-full px-4 py-3 bg-input border border-border rounded-lg text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent resize-none" rows="5" placeholder="Tell something about yourself"><?= $hero['short_bio'] ?></textarea>
                     ...
                     <div>
                         <label class="block text-sm font-medium text-foreground mb-2">Project Image (640 x 640) pixels</label>
                         <div class="space-y-3">
                             <input id="imageInput" name="image" accept="image/*" class="w-full px-4 py-2 bg-input border border-border rounded-lg text-foreground file:mr-4 file:py-2 file:px-3 file:bg-primary file:text-primary-foreground file:border-0 file:rounded-md file:cursor-pointer hover:file:bg-primary/90" type="file">
                         </div>
                     </div>
                 ...
                 <div class="grid max-lg:grid-cols-1 lg:grid-cols-2 gap-12 justify-center items-center">
                     <div class="space-y-6">
                         <div class="space-y-3 max-lg:text-center">
                             ...
                             <h1 id="fullNamePre" class="text-4xl md:text-5xl text-white lg:text-6xl font-bold text-foreground"><?= $hero['name'] ?></h1>
                             <h2 id="jobTitlePre" class="text-xl md:text-2xl text-blue-400 font-semibold"><?= $hero['job_title'] ?></h2>
                         </div>
                         <p id="shortBioPre" class="text-lg max-lg:text-center text-muted-foreground max-w-md leading-relaxed"><?= $hero['short_bio'] ?></p>
                     ...
                     <div class=" md:flex items-center justify-center">
                         <div class="relative w-full aspect-square max-w-sm">
                             ...
                             <div class="absolute inset-6 w-[336px] h-[336px] bg-gradient-to-tr from-accent/30 to-secondary/10 rounded-2xl flex items-center justify-center"><img id="imagePreview" class="rounded-2xl hover:scale-[1.1] duration-300" alt="" src="<?= $hero['image'] ?>"></div>
                         </div>
                     </div>
                 </div>
